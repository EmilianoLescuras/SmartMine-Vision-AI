...
Status: implemented · all acceptance criteria passing (7/7)
...
|---|----------|-----------|---------|
| AC-1 | `generate_notebooks.py` usa solo la API vigente de `WorkerCompliance` (`status`, `attributes`, `violations`); notebooks regenerados | ✅ implementado (via SPEC-001 AC-6) | SPEC-001 AC-6 |
| AC-2 | Ni el YAML ni `merge_datasets.py` escriben paths absolutos; el dataset se resuelve relativo al repo | ✅ implementado (via SPEC-001 AC-5) | SPEC-001 AC-5, SPEC-003 |
| AC-3 | Versión de Python única y consistente en `requirements.txt`, `environment.yml` y el generador | ✅ implementado (`LANG_INFO` corregido a 3.12) | — |
| AC-4 | Tests unitarios de `ppe_classifier` (SAFE/UNSAFE/UNKNOWN, IoU, center-containment, precedencia de señal embebida) | ✅ implementado (`tests/test_ppe_classifier.py`, 13 tests) | — |
| AC-5 | Conclusiones de notebooks reflejan estado real (marcadas `draft`/`previsión` o generadas de datos) — sin "Complete ✅" / "Clean" falsos | ✅ implementado (nb02 y nb06 corregidos) | — |
| AC-6 | `environment.yml` no asume hardware fijo (`cudatoolkit=11.8` condicional o documentado; coexiste con MPS/CPU) | ✅ implementado (`cudatoolkit` eliminado, comentado el por qué) | — |
| AC-7 | `LICENSE` presente (el README declara MIT pero el archivo no existe) | ✅ implementado (archivo MIT creado) | — |
